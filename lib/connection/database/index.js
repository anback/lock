  ...
  if (!assertMaybeArray(opts, 'additionalSignUpFields')) {
    ...
    additionalSignUpFields = additionalSignUpFields.reduce(function(r, x) {
      ...
      if (type !== 'hidden' && (typeof placeholder != 'string' || !placeholder)) {
        l.warn(
          opts,
          'Ignoring an element of `additionalSignUpFields` (' +
            name +
            ') because it does not contain a valid `placeholder` property. Every element of `additionalSignUpFields` must have a `placeholder` property that is a non-empty string.'
        );
        filter = false;
      }
      ...
      if (
        ...
        l.warn(
          ...
          'Ignoring an element of `additionalSignUpFields` (' +
            name +
            ') because it has a "select" `type` but does not specify an `options` property that is an Array or a function.'
        );
        filter = false;
      }
      if (type === 'hidden' && !value) {
        l.warn(
          ...
          'Ignoring an element of `additionalSignUpFields` (' +
            name +
            ') because it has a "hidden" `type` but does not specify a `value` string.'
        );
        filter = false;
      }
